...
/**
 *hello world bolera bolera boleraaa maya boleraa maya boler bijuli
 ...
 * @returns {HTMLDivElement}
 */
export function createMessagesWrapperElement(
	document,
	totalPages = 0,
	itemsPerPage = 0
) {
	...
	Object.defineProperty(messageWrapperElement, "scrollTop", {
		...
		set(newValue) {
			...
			messageWrapperElement.dispatchEvent(
				new document.defaultView.Event("scroll")
			);
		},
	});
	messageWrapperElement.currentPage = 0;
	for (let i = 0; i < itemsPerPage; i++) {
		const messageElement = createMessageElement.call(
			null,
			document,
			`Item ${i}`
		);
	...
	messageWrapperElement.addEventListener("scroll", (event) => {
		if (event.target.scrollTop === 0) {
			...
			if (hasNextPage) {
				...
				console.debug(
					"messageWrapperElement loading page",
					event.target.currentPage
				);
				console.debug(
...
export function createMessageElement(
	...
	element.innerHTML = `${
		includesUnsend ? "<span>You sent</span>" : ""
	}<span>${text}</span>`;
	element.addEventListener("mouseover", (event) => {
		console.debug(`message ${text} mouseover`);
		setTimeout(() => {
			const moreElement = event.target.ownerDocument.createElement("div");
			moreElement.setAttribute(
				"aria-label",
				"See more options for message from foo"
			);
			event.target.appendChild(moreElement);
			event.target.addEventListener("click", () => {
				// Listen for event of parent instead of moreElement because instagram use a svg Element
				...
				setTimeout(() => {
					if (event.target.messageActionsMenuElement) {
						...
						const messageActionsMenuElement = createMessageActionsMenuElement(
							document,
							includesUnsend,
							eventsTimeout
						);
						...
						event.target.ownerDocument.body.appendChild(
							messageActionsMenuElement
						);
					}
				}, eventsTimeout);
			});
 ...
 * @returns {HTMLDivElement}
 */
export function createMessageActionsMenuElement(
	document,
	includesUnsend = true
) {
